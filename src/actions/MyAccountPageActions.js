  ...
  static generateTransactionHistoryExportData() {
    return (dispatch, getState) => {
      ...
      if (accountId) {
        // Set loading status
        dispatch(
          MyAccountPagePrivateActions.setGenerateTransactionHistoryExportDataLoadingStatusAction(
            LoadingStatus.LOADING
          )
        );

        const periodType = getState().getIn(['myAccountPage', 'periodType']);
        const customTimeRangeStartDate = getState().getIn([
          'myAccountPage',
          'customTimeRangeStartDate'
        ]);
        const customTimeRangeEndDate = getState().getIn([
          'myAccountPage',
          'customTimeRangeEndDate'
        ]);
        ...
        const transactionHistory = HistoryService.convertRawHistoryToTransactionHistory(
          getState(),
          rawHistory
        );
        // Filter
        const filteredTransactionHistory = HistoryService.filterTransactionHistoryGivenTimeRange(
          transactionHistory,
          periodType,
          customTimeRangeStartDate,
          customTimeRangeEndDate
        ...
        const transactionHistoryExportData = HistoryService.convertTransactionHistoryToTransactionHistoryExportData( // eslint-disable-line
          getState(),
          filteredTransactionHistory
        );
        // Set export data
        dispatch(
          MyAccountPagePrivateActions.setTransactionHistoryExportDataAction(
            transactionHistoryExportData
          )
        );
        // Set loading status
