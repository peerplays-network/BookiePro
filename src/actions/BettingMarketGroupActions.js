...
class BettingMarketGroupPrivateActions {
  ...
  static setGetBettingMarketGroupsByEventIdsLoadingStatusAction(eventIds, loadingStatus) {
    return {
      type:
        ActionTypes.BETTING_MARKET_GROUP_SET_GET_BETTING_MARKET_GROUPS_BY_EVENT_IDS_LOADING_STATUS,
      eventIds,
      loadingStatus
    };
  }
...
class BettingMarketGroupActions {
  ...
  static getBettingMarketGroupsByIds(bettingMarketGroupIds) {
    return (dispatch, getState) => {
      ...
      let bettingMarketGroupsById = getState().getIn([
        'bettingMarketGroup',
        'bettingMarketGroupsById'
      ]);
      const persistedBettingMarketGroupsById = getState().getIn([
        'bettingMarketGroup',
        'persistedBettingMarketGroupsById'
      ]);
      bettingMarketGroupsById = bettingMarketGroupsById.concat(persistedBettingMarketGroupsById);
      const getBettingMarketGroupsByIdsLoadingStatus = getState().getIn([
        'bettingMarketGroup',
        'getBettingMarketGroupsByIdsLoadingStatus'
      ]);
      bettingMarketGroupIds.forEach((bettingMarketGroupId) => {
        if (
          getBettingMarketGroupsByIdsLoadingStatus.get(bettingMarketGroupId) === LoadingStatus.DONE
        ) {
          if (bettingMarketGroupsById.has(bettingMarketGroupId)) {
            retrievedBettingMarketGroups = retrievedBettingMarketGroups.push(
              bettingMarketGroupsById.get(bettingMarketGroupId)
            );
          }
        } else {
          idsOfBettingMarketGroupsToBeRetrieved = idsOfBettingMarketGroupsToBeRetrieved.push(
      ...
      if (idsOfBettingMarketGroupsToBeRetrieved.size === 0) {
        ...
        // Set status
        dispatch(
          BettingMarketGroupPrivateActions.setGetBettingMarketGroupsByIdsLoadingStatusAction(
            idsOfBettingMarketGroupsToBeRetrieved,
            LoadingStatus.LOADING
          )
        );
        return CommunicationService.getBettingMarketGroupsByIds(
          idsOfBettingMarketGroupsToBeRetrieved
        )
          .then((bettingMarketGroups) => {
            ...
            if (idsOfBettingMarketGroupsToBeRetrieved.size === bettingMarketGroups.size) {
              ...
              // Add to redux store
              dispatch(
                BettingMarketGroupActions.addOrUpdateBettingMarketGroupsAction(bettingMarketGroups)
              );
              // Set status
              dispatch(
                BettingMarketGroupPrivateActions.setGetBettingMarketGroupsByIdsLoadingStatusAction(
              ...
              const retrievedBettingMarketGroupIds = bettingMarketGroups
                .map((bettingMarketGroup) => bettingMarketGroup.get('id'));

              const filteredIdsOfBMGsToBeRetrieved = idsOfBettingMarketGroupsToBeRetrieved
                .filterNot((id) => retrievedBettingMarketGroupIds.includes(id));

              return CommunicationService.getPersistedBettingMarketGroupsByIds(
                filteredIdsOfBMGsToBeRetrieved
              ).then((persistedBMGs) => {
                retrievedBettingMarketGroups = retrievedBettingMarketGroups.concat(persistedBMGs);
                // Add to redux store
                dispatch(
                  BettingMarketGroupActions.addOrUpdateBettingMarketGroupsAction(
                    bettingMarketGroups
                  )
                );
                dispatch(
                  BettingMarketGroupActions.addPersistedBettingMarketGroupsAction(persistedBMGs)
                );
                // All fetched, set status
                dispatch(
                  BettingMarketGroupPrivateActions.setGetBettingMarketGroupsByIdsLoadingStatusAction( // eslint-disable-line
                    idsOfBettingMarketGroupsToBeRetrieved,
                    LoadingStatus.DONE
                  )
                );
          ...
          .then(() => {
            ...
            const assetIds = retrievedBettingMarketGroups
              .map((bettingMktGrp) => bettingMktGrp.get('asset_id'));

            return dispatch(AssetActions.getAssetsByIds(assetIds));
          })
          .then(() => retrievedBettingMarketGroups);
      }
    };
  }
  ...
  static getBettingMarketGroupsByEventIds(eventIds) {
    return (dispatch, getState) => {
      ...
      const bettingMarketGroupsById = getState().getIn([
        'bettingMarketGroup',
        'bettingMarketGroupsById'
      ]);
      let bettingMarketGroupsByEventId = Immutable.Map();
      bettingMarketGroupsById.forEach((bettingMarketGroup) => {
        const eventId = bettingMarketGroup.get('event_id');
        bettingMarketGroupsByEventId = bettingMarketGroupsByEventId.update(
          eventId,
          (bettingMarketGroups) => {
            if (!bettingMarketGroups) {
              bettingMarketGroups = Immutable.List();
            }
      ...
      const getBettingMarketGroupsByEventIdsLoadingStatus = getState().getIn([
        'bettingMarketGroup',
      ...
      eventIds.forEach((eventId) => {
        if (getBettingMarketGroupsByEventIdsLoadingStatus.get(eventId) === LoadingStatus.DONE) {
          if (bettingMarketGroupsByEventId.has(eventId)) {
            retrievedBettingMarketGroups = retrievedBettingMarketGroups.concat(
              bettingMarketGroupsByEventId.get(eventId)
            );
          }
        } else {
          eventIdsOfBettingMarketGroupsToBeRetrieved = eventIdsOfBettingMarketGroupsToBeRetrieved.push( // eslint-disable-line
            eventId
          );
        }
      });
      ...
      if (eventIdsOfBettingMarketGroupsToBeRetrieved.size === 0) {
        ...
        // Set status
        dispatch(
          BettingMarketGroupPrivateActions.setGetBettingMarketGroupsByEventIdsLoadingStatusAction(
            eventIdsOfBettingMarketGroupsToBeRetrieved,
            LoadingStatus.LOADING
          )
        );
        return CommunicationService.getBettingMarketGroupsByEventIds(
          eventIdsOfBettingMarketGroupsToBeRetrieved
        ).then((events) => {
