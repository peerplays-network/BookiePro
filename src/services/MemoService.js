...
class MemoService {
   ...
   * @param {*} encryptedMessage - The actual encrypted memo itself.
   *                               This is the message to be decoded.
  ...
  static decodeMemo(privateMemo, publicMemo, nonce, encryptedMessage) {
    // Using the AES library to decrypt the memo message using the private Memo key of the receiver
    // and the public memo key of the sender.
    let decryptedMessage = Aes.decrypt_with_checksum(
      privateMemo, publicMemo, nonce, encryptedMessage
    );
