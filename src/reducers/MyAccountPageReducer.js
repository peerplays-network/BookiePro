...
export default function(state = initialState, action) {
  switch (action.type) {
    case ActionTypes.MY_ACCOUNT_PAGE_PREPEND_TRANSACTION_HISTORY: {
      ...
      return state.update('transactionHistory', transactionHistory => transactionHistoryToBePrepended.concat(transactionHistory)); // eslint-disable-line
    }
    ...
    case ActionTypes.MY_ACCOUNT_PAGE_RESET_TRANSACTION_HISTORY_EXPORT_DATA: {
      return state.merge({
        generateTransactionHistoryExportDataLoadingStatus: initialState.get(
