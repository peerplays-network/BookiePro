 ...
 * 'b' represents minor number - soft update indicator provided that major number is the 
 * same during comparsion
 */
...
const SoftwareUpdateUtils = {
  ...
  /**
   * check if hard update action is necessary based on version string is in valid 
   * format. I.e. "a.b.c"
  ...
  /**
   * check if soft update action is necessary based on version string is in 
   * valid format. I.e. "a.b.c"
   * second item ("b") in splitted array [ a,b,c], from version string, represents minor number.
   *
   * if the minor number in current version is smaller than that in new version, and if both 
   * version strings have the same major number,
  ...
  isNeedSoftUpdate: newVersion => {
    ...
    return (
      newVersionMajorNumber === currentVersionMajorNumber &&
      newVersionMinorNumber === currentVersionMinorNumber &&
      newVersionPatchNumber > currentVersionPatchNumber
    );
  },
