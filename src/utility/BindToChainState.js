...
import utils from "./BlockchainUtils";
...
function checkIfRequired(t) {
  for(let k in ChainTypes) {
    if (Object.prototype.hasOwnProperty.call(ChainTypes, k)) {
      let v = ChainTypes[k];
    ...
    return class Wrapper extends React.Component {
      ...
      constructor(props) {
        ...
        for (let key in this.default_props) {
          if (Object.prototype.hasOwnProperty.call(this.default_props, key)) {
      ...
      update(next_props = null) {
        ...
        for( let key of this.chain_accounts )
        {
          let prop = props[key] || this.dynamic_props[key] || this.default_props[key];
          if(prop) {
            if(prop[0] === "#" && Number.parseInt(prop.substring(1), 10))
