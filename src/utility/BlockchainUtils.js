...
var Utils = {
  ...
  format_price: function(quoteAmount, quoteAsset, baseAmount, baseAsset, noSymbol,inverted = false, trailing_zeros = true) {
    ...
    if (inverted) {
      if (parseInt(quoteAsset.id.split(".")[2], 10) < parseInt(baseAsset.id.split(".")[2], 10)) {
        let value = (quoteAmount / precision) / (baseAmount / basePrecision);
        ...
        let value = (baseAmount / basePrecision) / (quoteAmount / precision);
        return `${this.format_number(value, Math.max(5, baseAsset.precision),trailing_zeros)}${!noSymbol ? "" + baseAsset.symbol + "/" + quoteAsset.symbol : ""}`;
      }
    } else {
      if (parseInt(quoteAsset.id.split(".")[2], 10) > parseInt(baseAsset.id.split(".")[2], 10)) {
        let value = (quoteAmount / precision) / (baseAmount / basePrecision);
  ...
  parse_float_with_comma: function(value) {
    ...
    if( value.length && isNaN(fvalue) && value !== "." )
      throw new Error("parse_float_with_comma: must be a number")
    else if( fvalue < 0 )
      return 0;
  ...
  get_odds_of_order(order) {
    const baseAssetId = order.getIn(['sell_price', 'base', 'asset_id']);
    const quoteAssetId = order.getIn(['sell_price', 'quote', 'asset_id']);
    let odds;
