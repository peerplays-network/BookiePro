...
class BannerClockDigits extends PureComponent {
  render() {
    ...
    for (let i = numberOfDigit; i > 0; i--) {
      const digit = (
        <BannerClockDigit key={ i } value={ Math.floor(value / Math.pow(10, i - 1)) % 10 } />
      );
      bannerClockDigitsContent.push(digit);
    }
    ...
    return (
      <div className='banner-clock-digits'>
        <div className='banner-clock-digits-content'>{bannerClockDigitsContent}</div>
        <div className='banner-clock-digits-footer'>{footerText}</div>
      </div>
    );
  }
}
