...
class AllSportsBanner extends PureComponent {
  ...
  renderTotalAmountStakedIntegerPart(totalAmountStaked) {
    let digits = [];
    const numberOfDigit = totalAmountStaked !== 0 ? (Math.floor(Math.log10(totalAmountStaked)) + 1) : 1;
  ...
  render() {
    ...
    const numberOfActiveEvents = this.props.globalBettingStatistics.get('number_of_active_events') || 0;
