...
export default class MetaexchangeDepositRequest extends React.Component {
  ...
  constructor(props) {
    ...
    // this.apiRoot = "http://localhost:1235/api"; this.marketPath =
    // "http://localhost:1235/markets/";
  }
  ...
  getModalId() {
    return '_asset_' + this.props.issuer_account.get('name') + '_'
  ...
  getMetaLink() {
    ...
    return this.marketPath + this.props.symbol_pair.replace('_', '/') + '?receiving_address='
      + encodeURIComponent(this.props.account.get('name') + ',' + withdrawAddr);
  }
  ...
  render() {
    ...
    if (!this.state.deposit_address) {
      try {
        let reply = wallet.deposit_keys[this.props.gateway][this.state.base_symbol][
          this.props.account.get('name')
        ];
        this.setState.deposit_address = reply.deposit_address;
        this.setState.memo = reply.memo;
      } catch (Error) {}
    }
    ...
    let deposit_modal_id = this.getDepositModalId(); /* eslint-disable-line */
