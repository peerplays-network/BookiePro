...
import MetaexchangeDepositRequest from './/metaexchange/MetaexchangeDepositRequest';
...
export default class MetaExchange extends React.Component {
  ...
  render() {
    ...
    let issuer_account = service === 'bridge'
      ? 'metaexchangebtc'
      : 'dev-metaexchange.monsterer';

    return (
      <div>
        <p style={ {fontSize: '1.1rem'} } className='label warning'>
          MetaExchange is closing down, please see this post for more info:
          <a
            href='https://metaexchange.info/closing'
            target='_blank'>https://metaexchange.info/closing {/* eslint-disable-line */}
          </a>
        </p>
        ...
        <div style={ {
          ...
          <TransactionWrapper
            fromAccount={ this.props.account.get('id') }
            to={ issuer_account }>
            {({to, fromAccount}) => {
              return (
                <RecentTransactions
