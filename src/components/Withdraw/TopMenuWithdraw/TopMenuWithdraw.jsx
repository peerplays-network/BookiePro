 ...
 *   hasWithdrawAmountErr : boolean to track if there are any custom errors in the 
 ...
 *   withdrawAmountErrMsg : the error text to display if there is any custom validation error on 
 * withdraw amount
 *   meta                 : contains metadata about the state of this field that 
 * redux-form is tracking.
 *     Some of the props that are used under meta prop are:
 *       touched          : true if the field has been touched. By default this will be set when 
 * the field is blurred.
 *       error            : The error for this field if its value is not passing validation.
 *                          Both synchronous, asynchronous, and submit validation errors will 
 * be reported here.
 */
const renderField = ({
  className,
...
class TopMenuWithdraw extends PureComponent {
  ...
  onWithdrawAmountChange(event) {
    ...
    if (!isNaN(withdrawAmount)) {
      if (
        parseFloat(withdrawAmount) > this.props.convertedAvailableBalance ||
  ...
  render() {
    const {
      ...
      isDisabled =
        ...
        asyncValidating ||
        pristine ||
        this.state.hasWithdrawAmountErr ||
        isWithdrawLoadingStatusLoading,
      prefix =
        currencyFormat === 'BTF'
          ? 'icon-bitcoin'
          : currencyFormat === 'mBTF'
          ...
          {isWithdrawLoadingStatusDone ? (
            <div className='withdraw-success-msg'>
              <p className='text-center'>
                {I18n.t('myAccount.withdraw_completed_msg_1')}{' '}
                <span className='withdraw-success-amount'>
                  <i className={ prefix } /> {withdrawAmount}
                </span>{' '}
                {I18n.t('myAccount.withdraw_completed_msg_2')}
              </p>
            </div>
          ) : (
            <form onSubmit={ handleSubmit } className='withdrawForm'>
              <div className={ 'form-fields bookie-amount-field ' + prefix }>
                <Field
                  name='topMenuWithdrawAmount'
                  id='topMenuWithdrawAmount'
                  ...
                  withdrawAmountErrMsg={
                    availableBalance !== -1
                      ? I18n.t('myAccount.insuffBitcoinErr') +
              ...
              <div className='form-fields'>
                <div className='card-footer'>
                  <Field
                    name='topMenuWalletAddr'
                    id='topMenuWalletAddr'
                    className='bookie-input walletAddr-input'
                    component={ renderField }
                  ...
                  <button
                    className={
                      'btn inputWithButton ' + (isDisabled ? 'btn-disabled' : ' btn-regular')
                    }
                    type='submit'
                    disabled={ isDisabled }
                  >
