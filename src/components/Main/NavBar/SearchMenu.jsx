 ...
 *
 *  User can click 'X' icon to clear the input text. It only appears when there is character
 * inputted
 *
...
class SearchOption extends PureComponent {
  ...
  render() {
    ...
    return (
      <div
        className={
          option.id !== '0' ? `Select-option-holder ${focus}` : 'Select-option-holder Select-result'
        }
        onClick={ option.id !== '0' ? this.handleMouseDown : null }
      >
        {option.is_live_market && <span className='badge' />}
        <div className={ option.id !== '0' ? this.props.className : 'Select-option Select-result' }>
          {this.props.option.isLiveMarket && <span className='indicator' />}
...
class SearchMenu extends PureComponent {
   ...
   * I.e. show MONEYLINE if moneyline exist, else show the first descendent in selected
    ...
    if (this.props.completeTree && event) {
      ...
      if (moneyline.size > 0) {
        ...
        this.props.navigateTo(
          '/betting/exchange/bettingmarketgroup/' +
            nested.getIn(keyPath).getIn(['children', 0, 'id'])
        );
      }
    }
  }
   ...
   *
   *  With Results : display the result counter ( index 0 in options[]) and list of active events
   * found ( index >= 1 in options[]).
   ...
   *            noResultsText in props is set as null as overriding the display condition of
   * noResultsText is needed
   */
  render() {
    ...
    const shouldShowOptions =
      ...
      (this.state.isLoading && this.props.searchResult.size === 0) ||
      value === RESULT_COUNT_ID ||
      (searchText && searchText.length === 0)
        ? []
        : //show search result when there is search result
        debounced && debounced.length > 0 && results.size > 1
          ? results.toJS()
