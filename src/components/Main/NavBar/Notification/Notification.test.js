...
describe('Test cases on "Notification" component',()=>{
  ...
  beforeEach(() => {
    ...
    NotificationWrapper = mount(<Provider store={ store }><Notification { ...props }/></Provider>);
  });
  ...
  it('Check Empty notification message', () => {
    expect(NotificationWrapper.find('.message').at(0).text()).toEqual(I18n.t('notification.empty'))
  });
});
...
describe('Test case for notificationItems rendered',()=>{
  ...
  const props = {
    ...
    notifications: new Immutable.List([
      Immutable.Map({
        id: 1.001, type: NotificationTypes.DEPOSIT, content :'Game of portland',
        date: new Date(), isRead: false}),
      Immutable.Map({
        id: 1.002, type: NotificationTypes.BET_RESOLVED, content :'Game of Hingkong',
        date: new Date(), isRead: false})])
  }
  beforeEach(() => {
    store = createStore(combineReducers({ form: formReducer }))
