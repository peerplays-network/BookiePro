...
class Export extends PureComponent {
    ...
    return (
      <Card className='export-card' title={ I18n.t('application.exportLoadingHeader') }>
        ...
        <div className='card-footer'>
          <button className='btn btn-cancel' onClick={ this.handleCancelClick }>
  ...
  renderExportReady() {
    ...
    if (exportData.size === 0) {
      return (
        <Card
          className='export-card export-empty-data'
          title={ I18n.t('application.exportNoDataHeader') }
        >
      ...
      return (
        <Card className='export-card' title={ I18n.t('application.exportDownloadHeader') }>
          <p>
            {I18n.t('application.exportDataReadyMsg1')}
            <br />
            {I18n.t('application.exportDataReadyMsg2')}
          </p>
          <div className='card-footer'>
            <button className='btn btn-cancel' onClick={ this.handleCancelClick }>
              {I18n.t('mybets.cancel')}
            </button>
            <button className='btn btn-primary download' onClick={ this.handleDownloadClick }>
              {I18n.t('application.download')}
            </button>
          </div>
        </Card>
      );
  ...
  render() {
    ...
    if (
      exportLoadingStatus === LoadingStatus.LOADING ||
      exportLoadingStatus === LoadingStatus.DONE
    ) {
        ...
        <div
          className={
            type === ExportTypes.RESOLVED_BETS
              ? 'export-overlay-top export-overlay'
              : 'export-overlay'
          }
        >
          {content}
