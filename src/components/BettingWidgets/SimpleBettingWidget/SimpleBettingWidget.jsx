...
const renderEventTime = (text, record) => {
  ...
  if (isLiveMarket) {
    return <span className='live'><span className=''/>{ I18n.t('object_status_enumerator.live') }</span>;
  } else {
  ...
  let columns = [
    {
      ...
      onCellClick: (record, event) => {
        record.get('bettingMarketGroupId') && navigateTo('/betting/exchange/bettingmarketgroup/' + record.get('bettingMarketGroupId'))
      }
    }, {
      title: '1',
      children: [{
