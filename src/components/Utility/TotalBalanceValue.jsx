            ...
            if (asset.get('id') !== coreAsset.get('id')) {
              setTimeout(() => {
                ...
                this.fromStatsIntervals[asset.get('id')] = setInterval(
                  MarketsActions.getMarketStats.bind(this, coreAsset, asset), 10 * 60 * 1000
                );
              }, 50);
            }
          }
        ...
        if (props.toAsset.get('id') !== coreAsset.get('id')) {
          ...
          setTimeout(() => {
            ...
            this.toStatsInterval = setInterval(
              MarketsActions.getMarketStats.bind(this, coreAsset, props.toAsset), 10 * 60 * 1000
            );
          }, 150);
        }
      }
    ...
    render() {
      ...
      for (let asset in assetValues) {
        if (assets[asset] && assetValues[asset]) {
          ...
          totalsTip += `<tr><td>${symbol}:&nbsp;</td><td style="text-align: right;">${amount} ${toAsset.get('symbol')}</td></tr>`; // eslint-disable-line
        }
      }

      // If any values are missing, let the user know.
      if (missingData) {
        totalsTip += `<tr><td>&nbsp;</td><td style="text-align: right;">${noDataSymbol} no data</td></tr>`; // eslint-disable-line
      }
      ...
      if (!inHeader) {
        return (
          <FormattedAsset
            ...
            decimalOffset={ toAsset.get('symbol').indexOf('BTC') === -1
...
class ValueStoreWrapper extends React.Component {
  ...
  render() {
    ...
    return (
      <TotalValue { ...this.props } toAsset={ preferredUnit }/>
    );
  }
}
...
class AccountWrapper extends React.Component {
    ...
    render() {
      ...
      this.props.accounts.forEach((account) => {
        ...
        if (account) {
          account.get('orders') && account.get('orders').forEach( (orderID) => {
          ...
          account.get('call_orders') && account.get('call_orders').forEach( (callID) => {
