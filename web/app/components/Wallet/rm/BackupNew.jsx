...
export class Create extends React.Component {
    ...
    renderDownloading(){
        let {sha1, name, size, content, lastModified} = this.props.backup,
            callback = ()=>{
                setBackupDate();
...
class LastBackupDate extends React.Component {
    render() {
        ...
        var backupTime;
        if(backupDate){
            backupTime = <h4><Translate content="wallet.last_backup" /> <FormattedDate value={backupDate}/></h4>;
        } else {
            backupTime = <Translate style={{paddingTop: 20}} className="facolor-error" component="p" content="wallet.never_backed_up" />;
        }
