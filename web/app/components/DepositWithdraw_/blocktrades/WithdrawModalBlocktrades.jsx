...
class WithdrawModalBlocktrades extends React.Component {
  ...
  constructor(props) {
    ...
    fetch(
      this.props.url + '/wallets/' + this.props.output_wallet_type + '/address-validator?address='
      + encodeURIComponent(
      ...
      reply.json().then((json) => {
        // only process it if the user hasn't changed the address since we initiated the
        // request
        let historyAddressLast = localStorage.getItem(
          `history_address_last_${this.props.output_wallet_type}`
        );

        if ((this.state.withdraw_address === historyAddressLast) &&
          historyAddressLast !== null
          ? localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`)
          : ''
        ) {
          this.setState({
    ...
    this.state = {
      ...
      withdraw_address: localStorage.getItem(
        `history_address_last_${this.props.output_wallet_type}`
      ) !== null
        ? localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`)
        : '',
      ...
      withdraw_address_selected: localStorage.getItem(
        `history_address_last_${this.props.output_wallet_type}`
      ) !== null
        ? localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`)
        : '',
  ...
  onSelectChanged(index, e) { /* eslint-disable-line */
    ...
    fetch(
      ...
      reply.json().then((json) => {
        ...
        if (this.state.withdraw_address === new_withdraw_address) {
          this.setState({
            withdraw_address_check_in_progress: false, withdraw_address_is_valid: json.isValid
          });
        }
      });
    });
  ...
  onWithdrawAddressChanged(e) {
    ...
    fetch(
      this.props.url + '/wallets/' + this.props.output_wallet_type
      + '/address-validator?address=' + encodeURIComponent(new_withdraw_address), {
        method: 'get',
        headers: new Headers({'Accept': 'application/json'})
      }).then((reply) => {
      reply.json().then((json) => {
    ...
    if ((
      ...
      if (!this.state.withdraw_address_is_valid) {
        ...
        if (localStorage.getItem(`history_address_${this.props.output_wallet_type}`) == null) {
          ...
          localStorage.setItem(
            `history_address_${this.props.output_wallet_type}`, JSON.stringify(withdrawals)
          );
        } else {
        ...
        localStorage.setItem(
          `history_address_last_${this.props.output_wallet_type}`, this.state.withdraw_address
        );
  ...
  onSubmitConfirmation() {
    ...
    AccountActions.transfer(
      ...
      asset.get('id'),
      this.state.memo
        ? this.props.output_coin_type + ':' + this.state.withdraw_address + ':' + new Buffer(this.state.memo, 'utf-8') /* eslint-disable-line */
        : this.props.output_coin_type + ':' + this.state.withdraw_address
      //this.props.output_coin_type + ":" + this.state.withdraw_address
    );
  }
  ...
  onDropDownList() {
    if (JSON.parse(
      localStorage.getItem(`history_address_${this.props.output_wallet_type}`)) != null
    ) {
      if (
        this.state.options_is_valid === false
      ) {
  ...
  render() {
    ...
    if (asset_types.length > 0) {
      ...
      if (current_asset_id) {
        balance = (
          <span><Translate component='span' content='transfer.available'/>:
            <BalanceComponent balance={ account_balances[current_asset_id] }/></span>
        );
      } else {
        balance = 'No funds';
      }
    ...
    if (this.state.options_is_valid) {
      options = <div className={ style_select }>
        {
          storedAddress
            .map(function (name, index) {
              return (
              <a key={ index } onClick={ this.onSelectChanged.bind(this, index) }> {/* eslint-disable-line */}
                  {name}
                </a>
              );
            }, this)
    ...
    if (!this.state.withdraw_address_check_in_progress &&
      ...
      // if (this.state.withdraw_address_is_valid)   invalid_address_message = <Icon
      // name="checkmark-circle" className="success" />; else
    ...
    if (this.props.output_supports_memos) {
      withdraw_memo = <div className='content-block'>
        ...
        <div className='grid-container'>
          <div className='content-block'>
            <h3><Translate
              ...
              symbol={ this.props.output_coin_symbol }/></h3>
          </div>
          <div className='content-block'>
            <AmountSelector
