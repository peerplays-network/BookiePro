...
/**
 * Given an account and an asset id, render a modal allowing modification of a
 * margin position for that asset
 *
 * Expected Properties:
...
class BorrowModalContent extends React.Component {
    ...
    _onBorrowChange(e) {
      ...
      let newState = {
        short_amount: amount,
        collateral: (this.state.collateral_ratio * (amount / feed_price))
    ...
    _onRatioChange(e) {
      ...
      let newState = {
        short_amount: this.state.short_amount,
        collateral: ((this.state.short_amount / feed_price) * ratio)
          .toFixed(this.props.backing_asset.get('precision')),
    ...
    _validateFields(newState) {
      ...
      let sqp = this.props.quote_asset.getIn(
        ['bitasset', 'current_feed', 'maximum_short_squeeze_ratio']
      ) / 1000;
      let mcr = this.props.quote_asset.getIn(
        ['bitasset', 'current_feed', 'maintenance_collateral_ratio']
      ) / 1000;

      if (
        parseFloat(newState.collateral_ratio)< (this._isPredictionMarket(this.props)
          ? 1
          : sqp * mcr)
      ) {
    ...
    _onSubmit(e) {
      ...
      tr.add_type_operation('call_order_update', {
        ...
        'delta_collateral': {
          'amount': parseInt(
            this.state.collateral * backingPrecision - currentPosition.collateral, 10
          ),
      ...
      if (props && props.hasCallOrders && props.call_orders) {
        for (let key in props.call_orders) {
          if (props.call_orders.hasOwnProperty(key) && props.call_orders[key]) {
            if (
              props.quote_asset.get('id') === props.call_orders[key]
                .getIn(['call_price', 'quote', 'asset_id'])
            ) {
    ...
    _getFeedPrice() {
      ...
      return 1 / utils.get_asset_price(
        this.props.quote_asset.getIn(
          ['bitasset', 'current_feed', 'settlement_price', 'quote', 'amount']
        ),
        this.props.backing_asset,
        this.props.quote_asset.getIn(
          ['bitasset', 'current_feed', 'settlement_price', 'base', 'amount']
        ),
    ...
    _calculateCallPrice() {
      let CALL_PRICE = 0;
      let maintenanceRatio = this.props.quote_asset.getIn(
        ['bitasset', 'current_feed', 'maintenance_collateral_ratio']
      ) / 1000;

      let asset_price = utils.get_asset_price(
        this.props.short_amount,
        this.props.quote_asset,
      ...
      if (this.props.quote_asset) {
        CALL_PRICE = (1 / maintenanceRatio) * (1 / asset_price);
      }
    ...
    render() {
      ...
      let buttonClass = classNames(
        'button', {disabled: errors.collateral_balance || !isValid}, {success: isValid}
      );
      ...
      let squeezeRatio = this.props.quote_asset.getIn(
        ['bitasset', 'current_feed', 'maximum_short_squeeze_ratio']
      ) / 1000;
      let CALL_PRICE = this._calculateCallPrice(); // eslint-disable-line
                  ...
                  <div style={ {paddingBottom: '1rem'} } >
                    <div className='borrow-price-feeds'>
                      ...
                      <FormattedPrice
                        ...
                        quote_asset={ quote_asset.getIn(
                          ['bitasset', 'current_feed', 'settlement_price', 'base', 'asset_id']
                        ) }
                        base_asset={ quote_asset.getIn(
                          ['bitasset', 'current_feed', 'settlement_price', 'quote', 'asset_id']
                        ) }
                    ...
                    <div className='borrow-price-feeds'>
                      ...
                      <FormattedPrice
                        ...
                        base_amount={ squeezeRatio * quote_asset.getIn(
                          ['bitasset', 'current_feed', 'settlement_price', 'quote', 'amount']
                        ) }
                      />
                    </div>
                    ...
                    <div className='borrow-price-final'>
                      ...
                      {this.state.newPosition ?
                        <FormattedPrice
                          style={ {fontWeight: 'bold'} }
                          quote_amount={
                            maintenanceRatio * this.state.short_amount * quotePrecision
                          }
