...
class ExchangeService {
    ...
    static fetchChartDepthData(quoteAssetSymbol, coreAssetSymbol) {
        ...
        return Promise.all([Repository.getAsset(quoteAssetSymbol), Repository.getAsset(coreAssetSymbol)]).then(([quoteAsset, baseAsset]) => {
            ...
            return OrderRepository.fetchLimitOrders(baseAsset.id, quoteAsset.id).then((limitOrders) => {
                ...
                bids.reverse().forEach((order, idx) => {
                ...
                return ExchangeMarketService.getSettlementPrice(baseAsset, quoteAsset).then((settlementPrice) => {
    ...
    static fetchChartData(quoteAssetSymbol, coreAssetSymbol, bucketSize) {
        ...
        return AssetRepository.fetchAssetsByIds([quoteAssetSymbol, coreAssetSymbol], true).then((results) => {
            ...
            return Promise.all([HistoryRepository.fetchMarketHistory(baseAsset.id, quoteAsset.id, bucketSize, startDate.toISOString().slice(0, -5), endDate.toISOString().slice(0, -5)).then((result) => {
    ...
    static _priceChart(priceHistory, bucketSize, baseAsset, quoteAsset) {
        ...
        for (let i = 0; i < priceHistory.length; i++) {
            ...
            if (quoteAsset.id === priceHistory[i].key.quote) {
                high = utils.get_asset_price(priceHistory[i].high_base, baseAsset, priceHistory[i].high_quote, quoteAsset);
                low = utils.get_asset_price(priceHistory[i].low_base, baseAsset, priceHistory[i].low_quote, quoteAsset);
                open = utils.get_asset_price(priceHistory[i].open_base, baseAsset, priceHistory[i].open_quote, quoteAsset);
                close = utils.get_asset_price(priceHistory[i].close_base, baseAsset, priceHistory[i].close_quote, quoteAsset);
                volume = utils.get_asset_amount(priceHistory[i].quote_volume, quoteAsset);
            } else {
                low = utils.get_asset_price(priceHistory[i].high_quote, baseAsset, priceHistory[i].high_base, quoteAsset);
                high = utils.get_asset_price(priceHistory[i].low_quote, baseAsset, priceHistory[i].low_base, quoteAsset);
                open = utils.get_asset_price(priceHistory[i].open_quote, baseAsset, priceHistory[i].open_base, quoteAsset);
                close = utils.get_asset_price(priceHistory[i].close_quote, baseAsset, priceHistory[i].close_base, quoteAsset);
                volume = utils.get_asset_amount(priceHistory[i].base_volume, quoteAsset);
            }
        ...
        if (priceLength > 0 && priceLength < 200) {
            ...
            if (prices[priceLength - 1][0] !== finalDate) {
                if (priceLength === 1) {
                    prices.push([finalDate - bucketSize * 1000, prices[0][4], prices[0][4], prices[0][4], prices[0][4]]);
            ...
            for (let ii = 0; ii < prices.length - 1; ii++) {
                if (prices[ii+1][0] !== (prices[ii][0] + bucketSize * 1000)) {
                    ...
                    prices.splice(ii + 1, 0, [prices[ii][0] + bucketSize * 1000, prices[ii][4], prices[ii][4], prices[ii][4], prices[ii][4]]);
    ...
    static fetchTabStats(mainAssetSymbol, topMarkets, assetsCacheBySymbol) {
        ...
        topMarkets.forEach(market => {
            ...
            if (market !== mainAssetSymbol && assetsCacheBySymbol[mainAssetSymbol] && assetsCacheBySymbol[market]) {
                quoteAssetsSymbolIndexes.push(market);
                promises.push(Promise.all([
                    HistoryRepository.fetchMarketHistory(assetsCacheBySymbol[mainAssetSymbol].id, assetsCacheBySymbol[market].id, 3600, startDateShort.toISOString().slice(0, -5), endDate.toISOString().slice(0, -5)),
                    HistoryRepository.fetchFillOrderHistory(assetsCacheBySymbol[mainAssetSymbol].id, assetsCacheBySymbol[market].id, 1)
                ]));
            }
        ...
        return Promise.all(promises).then((results) => {
            ...
            results.forEach((res, idx) => {
                ...
                let calcVolumeAndChange = ExchangeService.calcVolumeAndChange(marketHistoryData, assetsCacheBySymbol[mainAssetSymbol], assetsCacheBySymbol[marketSymbol]);
                ...
                if (fillOrderHistoryData.length) {
                    let priceObject = ExchangeService.getPriceObjectByOrder((fillOrderHistoryData && fillOrderHistoryData.length) ? fillOrderHistoryData[0]['op'] : null, assetsCacheBySymbol[mainAssetSymbol], assetsCacheBySymbol[marketSymbol]);
                ...
                if (!finalPrice) {
                    let price = utils.convertPriceObj(assetsCacheBySymbol[marketSymbol], assetsCacheBySymbol[mainAssetSymbol]);
                    finalPrice = utils.get_asset_price(price.base.amount, assetsCacheBySymbol[mainAssetSymbol], price.quote.amount, assetsCacheBySymbol[marketSymbol]);
                }
                ...
                marketRows.push({
                    ...
                    'price': finalPrice,
                    'priceText': utils.format_number(finalPrice, finalPrice > 1000 ? 0 : finalPrice > 10 ? 2 : precision)
                });
    ...
    static fetchExchangeData(quoteAssetSymbol, coreAssetSymbol) {
        return AssetRepository.fetchAssetsByIds([quoteAssetSymbol, coreAssetSymbol], true).then((results) => {
            ...
            return Promise.all([
                HistoryRepository.fetchMarketHistory(baseAsset.id, quoteAsset.id, 3600, startDateShort.toISOString().slice(0, -5), endDate.toISOString().slice(0, -5)),
                ...
                let calcVolumeAndChange = ExchangeService.calcVolumeAndChange(marketHistoryData, baseAsset, quoteAsset);
                ...
                let latestPriceObject = ExchangeService.getLatestChangePriceObject(activeMarketHistory, baseAsset, quoteAsset);
    ...
    static getLatestChangePriceObject(activeMarketHistory, baseAsset, quoteAsset) {
        ...
        if (activeMarketHistory.size) {
            ...
            latestPriceFull = utils.get_asset_price(latest.receives.amount, receivesAsset, latest.pays.amount, paysAsset, isAsk);
            ...
            if (second_latest) {
                ...
                let oldPriceFull = utils.get_asset_price(second_latest.receives.amount, receivesAsset, second_latest.pays.amount, paysAsset, isAsk);

                changeClass = latestPriceFull === oldPriceFull ? "" : latestPriceFull - oldPriceFull > 0 ? "up" : "down";
    ...
    static getPriceObjectByOrder(order, baseAsset, quoteAsset) {
        ...
        if (order) {
            ...
            latestPriceFull = utils.get_asset_price(order.receives.amount, receivesAsset, order.pays.amount, paysAsset, isAsk);
