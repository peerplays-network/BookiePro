...
class TransactionService {
  ...
  static commitMove(tournamentId, gameId, playerAccountId, commit, privateKey) {
    return Repository.getObject(tournamentId).then((tournament) => {
      ...
      return RPSRepository.commitMove(
        gameId,
        tournament.getIn(['options', 'buy_in', 'asset_id']),
        playerAccountId,
        commit,
        privateKey
      );
    ...
    return Repository.getObject(tournamentId).then((tournament) => {
      ...
      return iDB.load_data('wallet').then(([wallet]) => {
        if (wallet) {
          ...
          if (revealMove) {
            return RPSRepository.broadcastRevealMove(
              gameId,
              tournament.getIn(['options', 'buy_in', 'asset_id']),
              playerAccountId,
              revealMove,
            ...
            console.log(
              'Error no stored reveal move for commit move, game: %o, player: %o, commitMove: %O',
              gameId,
              playerAccountId,
              commitMove
            );
