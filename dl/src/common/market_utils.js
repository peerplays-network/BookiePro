...
class MarketUtils {
  ...
  static parseOrder(order, base, quote, invert = false) {
    ...
    let quotePrecision = utils.get_asset_precision(quote.toJS
      ? quote.get('precision')
      : quote.precision);
    let basePrecision = utils.get_asset_precision(base.toJS
      ? base.get('precision')
      : base.precision);
  ...
  static parse_order_history(order, paysAsset, receivesAsset, isAsk, flipped) { // eslint-disable-line
    ...
    let payPrecision = utils.get_asset_precision(paysAsset.toJS
    ...
    let price = utils.price_to_text(price_full, isAsk ? receivesAsset : paysAsset, isAsk
      ? paysAsset : receivesAsset, isAsk ? paysAsset.precision
      : receivesAsset.precision);
    let amount = utils.price_to_text((isAsk ? receives : pays), isAsk
      ? paysAsset
      : receivesAsset, isAsk ? receivesAsset : paysAsset, isAsk
      ? receivesAsset.precision
      : paysAsset.precision);
    let total = utils.price_to_text((isAsk ? pays : receives), isAsk
      ? receivesAsset
      : paysAsset, isAsk ? paysAsset : receivesAsset, isAsk
      ? paysAsset.precision
      : receivesAsset.precision );
  ...
  static isMarketAsset(quote, base) {
    ...
    if (quote.get('bitasset') && base.get('id') === quote
      .getIn(['bitasset', 'options', 'short_backing_asset'])) {
      isMarketAsset = true;
      marketAsset = {id: quote.get('id')};
    } else if (base.get('bitasset') && quote.get('id') === base
      .getIn(['bitasset', 'options', 'short_backing_asset'])) {
      inverted = true;
      isMarketAsset = true;
