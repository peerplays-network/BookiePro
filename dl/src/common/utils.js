...
var Utils = {
    ...
    price_to_text: function(price, base, quote, forcePrecision = null, s = null) {
    ...
    convertPriceObj: function(fromRate, toRate, fromID, toID) {
        ...
        if (this.is_object_type(fromRate.id, "asset")) {
            fromID = fromRate.id;
            fromRate = fromRate.bitasset ? fromRate.bitasset.current_feed.settlement_price : fromRate.options.core_exchange_rate;
        }
        ...
        if (this.is_object_type(toRate.id, "asset")) {
            toID = toRate.id;
            toRate = toRate.bitasset ? toRate.bitasset.current_feed.settlement_price : toRate.options.core_exchange_rate;
        }
    ...
    convertValue: function(priceObject, amount, fromAsset, toAsset) {
        ...
        let quotePrecision = this.get_asset_precision(fromAsset.toJS ? fromAsset.get("precision") : fromAsset.precision);
        let basePrecision = this.get_asset_precision(toAsset.toJS ? toAsset.get("precision") : toAsset.precision);

        let assetPrice = this.get_asset_price(priceObject.quote.amount, fromAsset, priceObject.base.amount, toAsset);

        let eqValue =
            (fromAsset.toJS ? fromAsset.get("id") : fromAsset.id) !== (toAsset.toJS ? toAsset.get("id") : toAsset.id)
                ?
            (basePrecision * (amount / quotePrecision) / assetPrice)
                :
            amount;
