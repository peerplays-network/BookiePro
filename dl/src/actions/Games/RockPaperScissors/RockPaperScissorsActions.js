...
import {PrivateKey, ChainTypes, ChainStore} from "peerplaysjs-lib";
                    ...
                    switch (tab) {
                        ...
                        case 'rps':
                            ...
                            if (state.rockPaperScissorsReducer.activeGameId) {
                                dispatch(RockPaperScissorsActions.setGame(state.rockPaperScissorsReducer.activeGameId));
                            }
        ...
        return (dispatch, getState) => {
            ...
            Repository.getAccount(state.app.account).then((account) => {
                ...
                return Repository.getAsset(tournament.getIn(['options', 'buy_in', 'asset_id']))
                    ...
                    TournamentTransactionService.joinToTournament(operationJSON, accountJs, asset.toJS(), () => {
    ...
    static joinToLastTournament(accountId) {
        ...
        return (dispatch) => {
            ...
            Repository.getAccount(accountId).then((account) => {
                ...
                Repository.getTournamentIdsInStateDirectly(accountId, 'accepting_registrations').then((results) => {
                    ...
                    results.forEach((element) => {
                        let id = parseFloat(element.id.replace('1.' + ChainTypes.object_type.tournament + '.', ''));
                    ...
                    if (maxElement) {
                        ...
                        Promise.all([Repository.getAccount(account.get('id')), Repository.getAsset(operationJSON.buy_in.asset_id)]).then((data) => {
        ...
        return (dispatch, getState) => {
            ...
            Repository.getAsset(data.buy_in_asset_symbol).then((asset) => {
                ...
                let tr = wallet_api.new_transaction(),
                    precision = utils.get_asset_precision(asset.get("precision")),
                ...
                Repository.getAccount(state.app.account).then((account) => {
                    ...
                    let operationJSON = {
                        ...
                        "options": {
                            ...
                            "buy_in": { amount: buy_in_in_satoshis, asset_id: asset.get("id") },
                    ...
                    Promise.all([Repository.getAccount(operationJSON.creator), Repository.getAsset(operationJSON.options.buy_in.asset_id)]).then(([payer, asset]) => {
                        ...
                        if (payer && asset) {
                            ...
                            TournamentTransactionService.createTournament(operationJSON, data.whitelist, payer, asset, () => {
    ...
    static fetchExploreTournaments(page = null) {
        return (dispatch, getState) => {
            ...
            Repository.getAccount(state.app.accountId).then((account) => {
                ...
                let shortLastTournamentId = parseFloat(lastTournamentId.substring(tournament_prefix.length));
                ...
                let last =  tournament_prefix + Math.max((shortLastTournamentId - exploreCountPerPage * page) + 1, 0),
                 ...
                 * Cache
                 */
                if (currentLastTournamentId === lastTournamentId && currentLastTournamentPage === page && currentExploreList.size) {
                ...
                return Promise.all([ objects, Promise.all(promises), Promise.all(promisesAssets), Promise.all(promisesCreators) ]);
                ...
                tournaments.forEach((tournament, idx) => {
                    ...
                    if (assetsHash[tournament.getIn(['options', 'buy_in', 'asset_id'])]) {
                        tournament = tournament.setIn(['options', 'buy_in', 'asset'], assetsHash[tournament.getIn(['options', 'buy_in', 'asset_id'])]);
                    }

                    if (creatorHash[tournament.getIn(['creator'])]) {
                        tournament = tournament.setIn(['creator_account_name'], creatorHash[tournament.getIn(['creator'])].get('name'));
                    }

                    tournament = tournament.setIn(["options", "registration_deadline"], TimeHelper.timeStringToDate(tournament.getIn(["options", "registration_deadline"])));
                    ...
                    if (detail) {
                        ...
                        if (exploreDropDownCurrent === 'any' || exploreDropDownCurrent === tournament.getIn(['options', 'buy_in', 'asset_id'])) {
                            exploreList = exploreList.push(tournament.set('tournament_details', detailsHash[tournament.get('tournament_details_id')]));
                        }
                ...
                exploreList = TournamentSortService.sortTournamentList(exploreList, state.rockPaperScissorsReducer.exploreSortColumn, state.rockPaperScissorsReducer.exploreSortDirection, account);
    ...
    static setSortExploreBy(field) {
        ...
        return (dispatch, getState) => {
            ...
            if (state.rockPaperScissorsReducer.exploreSortColumn === field) {
                direction = (state.rockPaperScissorsReducer.exploreSortDirection === 'desc') ? 'asc' : 'desc';
            } else {
            ...
            Repository.getAccount(state.app.account).then((account) => {
                ...
                tournaments.forEach((tournament) => {
                    ...
                    if (detail) {
                        ...
                        if (!fPlayer && (findDropDownCurrent === 'any' || findDropDownCurrent === tournament.getIn(['options', 'buy_in', 'asset_id']))) {
                            ...
                            if (needAdd) {
                                findList = findList.push(tournament.set('tournament_details', detailsHash[tournament.get('tournament_details_id')]));
                            }
                        }
                    }
    ...
    static fetchDashboardTournaments() {
        return (dispatch, getState) => {
            ...
            Repository.getAccount(state.app.account).then((account) => {
                ...
                return Promise.all([Repository.getTournamentIdsInState(null, 'accepting_registrations'), Repository.getTournamentIdsInState(null, 'awaiting_start'), Repository.getTournamentIdsInState(null, 'in_progress')]);

            }).then(([rawUpcomingTournamentIds, rawTournamentIdsAwaitingStart, rawTournamentIdsInProgress]) => {

                let allUpcomingTournamentIds = rawUpcomingTournamentIds.union(rawTournamentIdsAwaitingStart),
                    promises = [];
                ...
                tournaments.forEach((tournament) => {
                    ...
                    if (detail) {
                        ...
                        if (needAdd) {
                            dashboardList = dashboardList.push(tournament.set('tournament_details', detailsHash[tournament.get('tournament_details_id')]));
                        }
                ...
                switch (tournament.get('state')) {
                    case 'in_progress':
                        ...
                        let match = matches.find((match) => {
                            ...
                            return match.get('state') === 'match_in_progress' && match.get('players') && match.get('players').find((playerId) => {
                        ...
                        if (match) {
                            ...
                            return Promise.all([Promise.all(
                                ...
                                .then(([players, games]) => {
                                    ...
                                    games.forEach((game) => {
                                        if (!previousGamesHash[game.get('id')] || (game !== previousGamesHash[game.get('id')] && game.get('state') !== previousGamesHash[game.get('id')].get('state'))) {
                                    ...
                                    let item = Immutable.Map({
                                        ...
                                        games: Immutable.List(match.getIn(['games']).map((gameId) => {
                                            ...
                                            if (game) {
                                                ...
                                                if ((moves.get(1) != null) && (moves.get(0) != null) && (gesture0 == gesture1)) {
                                                    winnerText = 'tie';
                                                }
            ...
            return dispatch(RWalletUnlockNewActions.getKeyFromState('active')).then((privateKey) => {
                ...
                return iDB.load_data('wallet').then(([wallet]) => {
                    ...
                    if (wallet) {
                        ...
                        wallet.reveal_moves[gameId][playerAccountId] = wallet.reveal_moves[gameId][playerAccountId] || {};
                    ...
                    return RPSTransactionService.commitMove(tournamentId, gameId, playerAccountId, commit, privateKey, dispatch)
                });
