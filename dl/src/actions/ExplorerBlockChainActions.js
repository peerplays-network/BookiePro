...
class ExplorerBlockChainActions {
	...
	static updateAllData() {
		return (dispatch, getState) => {
			...
			Promise.all([Repository.getObject('2.1.0'), Repository.getObject('2.0.0'), Repository.getObject('1.3.0')]).then(([object210, object200, coreAsset]) => {
					...
					if(recentBlocks.get(0).id > latestBlock) {
						...
						recentOperations.toJS().forEach(operation => {
							if(increment < MAX_LATEST_OPERATIONS) {
								...
								neededObjects = ExplorerBlockChainService.setNeededObjects(operation, neededObjects);
						...
						let offset_mills = new Date().getTime() - TimeHelper.timeStringToDate(state.explorerBlockchainPage.time).getTime();
						let updatedAt = new Date(state.explorerBlockchainPage.time).getTime() + offset_mills;// * 0.75;
						...
						Promise.all([]).then(() => {
							...
							Promise.all((filteredKeys.length) ? [ObjectRepository.fetchObjectsByIds(filteredKeys)] : []).then((data) => {
								...
								operationsNew.forEach((newOperation) => {
									...
									let updatedOperation = ExplorerBlockChainService.setOperationField(newOperation, operation, fetchedObjects);
								...
								operations = operations.map((operation) => {
									operation.deltaBlocks = (operation.block_id - last_irreversible_block_num > 0) ? operation.block_id - last_irreversible_block_num : 0;
									return operation;
								});
